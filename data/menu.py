...
class GameModeMenu(Menu):
    def __init__(self, game):
        Menu.__init__(self, game)
        self.game_mode_menu_button = ImportGameModeMenuButton(self)
    ...
    def display_menu(self):
        ...
        while self.run_display:
            self.bg_loader(self.bg_img, 1)
            self.game.check_events()
            self.game_mode_menu_button.import_game_mode_menu_button()
            self.blit_canvas()
...
class HintMenu(Menu):
    def __init__(self, game):
        ...
        self.bg_img = pygame.image.load(f'{self.bg_img_loc}/instructions2.png').convert_alpha()
